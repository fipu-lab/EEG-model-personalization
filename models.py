...
def KAN_CNN_1D_model(num_classes=4, chans=16, samples=60):
    input_shape = (samples, chans)
    model = Sequential(
        layers=[
            tfkan.layers.Conv1DKAN(
                16, (3), input_shape=input_shape, kan_kwargs={"basis_activation": "swish"}
            ),
            MaxPooling1D((3)),
            Flatten(),
...
def KAN_CNN_1D_BN_model(num_classes=4, chans=16, samples=60):
    input_shape = (samples, chans)
    model = Sequential(
        layers=[
            tfkan.layers.Conv1DKAN(
...
def CNN_1D_BN_model(num_classes=4, chans=16, samples=60):
    input_shape = (samples, chans)
    model = Sequential(
        layers=[
            Conv1D(16, (3), activation='relu', input_shape=input_shape),
